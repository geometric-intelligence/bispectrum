...
def reconstruct_batch(
    ...
    for step in range(n_steps):
        ...
        with torch.no_grad(), torch.amp.autocast('cuda', enabled=False):
            ...
            per_sample_post = (diff_post.real**2 + diff_post.imag**2).sum(dim=-1) / target_norm_sq
...
def run_demo(
    ...
    sht = RealSHT(nlat, nlon, lmax=lmax + 1, mmax=lmax + 1, grid='equiangular', norm='ortho').to(
        device
    )
    isht = InverseRealSHT(
    ...
    for d_idx in range(n_digits):
        ...
        for k, val in enumerate(invariance_rel.cpu().tolist()):
            ...
            logger.info(
                '  invariance ||beta(f_%d) - beta(f_0)|| / ||beta(f_0)|| = %.3e (%s)', k, val, tag
            )
        ...
        target_norm_sq = (beta_stack.real**2 + beta_stack.imag**2).sum(dim=-1).clamp_min(1e-30)
        ...
        for restart in range(max(n_recon_restarts, 1)):
            ...
            with torch.no_grad(), torch.amp.autocast('cuda', enabled=False):
                ...
                per_target_rel_sq = (diff.real**2 + diff.imag**2).sum(dim=-1) / target_norm_sq
...
def dump_results_json(results: list[ReconResult], meta: dict[str, object], path: Path) -> None:
    """Persist per-record metrics + meta.

    Tensors are dropped (kept in figures).
    """
...
def _sphere_grid_lines(
    ...
    Used to draw a faint reference graticule on the orthographic sphere so the eye can pick up
    rotation differences between panels.
    """
    e = np.deg2rad(elev_deg)
    a = np.deg2rad(azim_deg)
    cam = np.array([np.cos(e) * np.cos(a), np.cos(e) * np.sin(a), np.sin(e)])
...
def _draw_sphere(
    ...
    circle = plt.Circle((0, 0), 1.0, fill=False, color='black', linewidth=0.6, zorder=3)
    ax.add_patch(circle)
...
def _draw_panel(
    ...
    if render == 'sphere':
        _draw_sphere(ax, f, vmin, vmax, view_size, elev_deg, azim_deg, grid_lines=grid_lines)
    else:
        ax.imshow(
            f,
            cmap='RdBu_r',
...
def make_orbits_figure(
    ...
    fallback_lines = _sphere_grid_lines(elev_deg, azim_deg) if render == 'sphere' else None

    def _view_for(
        arr: np.ndarray,
    ) -> tuple[float, float, list[tuple[np.ndarray, np.ndarray]] | None]:
        if render != 'sphere':
    ...
    for d in range(n_digits):
        ...
        for k in range(n_targets):
            ...
            if k == 0:
                ax_t.set_ylabel(f'#{d}', rotation=0, ha='right', va='center', fontsize=8)
            ...
            if d == 0:
                ...
                ax_a.set_xlabel(f'img {r.aligned_image_space_rel:.1e}', fontsize=6)
...
def make_convergence_figure(results: list[ReconResult], path: Path) -> None:
    """Median + IQR envelope of the relative bispectrum residual vs.
...
def make_invariance_figure(results: list[ReconResult], path: Path) -> None:
    """Scatter of invariance check vs.

    recon residual per (digit, rot).
    """
    plt.rcParams.update(NEURIPS_RCPARAMS)
...
def make_paper_figure(
    ...
    for d in digit_indices:
        if d not in by_digit:
            raise ValueError(f'digit_idx {d} not in results (available: {sorted(by_digit)})')
    ...
    for j, r in enumerate(picks):
        ...
        _draw_sphere(axes[0][j], target, vmin, vmax, view_size, e_t, a_t, grid_lines=gl_t)
        _draw_sphere(axes[1][j], aligned, vmin, vmax, view_size, e_t, a_t, grid_lines=gl_t)
...
def print_summary(results: list[ReconResult]) -> None:
    ...
    print(
        f'  image (aligned)||R . f_hat - target|| / ||target||             : {_stats(aligned_all)}'
    )
    print(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--align_n_steps', type=int, default=200, help='Adam steps per alignment restart.'
    )
    parser.add_argument(
        '--align_lr',
        type=float,
    ...
    parser.add_argument(
        '--elev_deg', type=float, default=25.0, help='Fallback / fixed-view elevation in degrees.'
    )
    parser.add_argument(
        '--azim_deg', type=float, default=30.0, help='Fallback / fixed-view azimuth in degrees.'
    )
    parser.add_argument(
        '--fixed_view',
        action='store_true',
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help='Override output path for the paper figure (default: <output_dir>/paper_orbits.pdf).',
    )
    parser.add_argument(
        '--paper_only',
        action='store_true',
...
def main() -> int:
    ...
    paper_digits = sorted({int(d) for d in args.paper_digits})
    if args.paper_only:
        if not paper_digits:
