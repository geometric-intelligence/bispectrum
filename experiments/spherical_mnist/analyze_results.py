...
def plot_data_efficiency(
    ...
    for ax, (train_mode, test_fn, title) in zip(axes, panels, strict=False):
...
def plot_combined_figure(
    ...
    models_with_data = [m for m in MODEL_ORDER if grouped.get((m, 'C')) or grouped.get((m, 'R'))]
...
def _plot_cohen_panel(
    ...
    for i, proto in enumerate(protocols):
        ...
        for j, (bar, v) in enumerate(zip(bars, vals, strict=False)):
            if v <= 0:
                continue
            show_label = True
            if i > 0 and v > 0.2:
