...
def _canonical_train_mode(mode: str) -> str:
    """Map legacy `NR` to canonical `C`.

    Pre-normalized runs default to `R`.
    """
    if mode in {'C', 'NR'}:
        return 'C'
    if mode == 'R':
...
def _train_size_value(record: dict) -> int:
    """Resolve a record's training-subset size.

    Treat None / <=0 / missing as full.
    """
...
def main() -> None:
    ...
    labels_matched = [f'{MODEL_STYLE[m[0]]["label"]} ({m[1] // 1000}K)' for m in matched_data]
    ...
    for bar, auc, err in zip(bars, aucs_matched, errs_matched, strict=False):
        label = f'{auc:.3f}'
        if err > 0:
            label += f'\u00b1{err:.3f}'
        ax2.text(
    ...
    for model in MODEL_ORDER:
        ...
        print(
            f'   {MODEL_STYLE[model]["label"]:25s}  best AUC={best["mean_auc"]:.4f}{std_str} at {best["n_params"]:>10,} params ({best["n_seeds"]} seeds)'
        )
    ...
    for model in MODEL_ORDER:
        ...
        if len(pts) >= 2:
            ...
            print(
                f'   {MODEL_STYLE[model]["label"]:25s}  {pts[0]["mean_auc"]:.4f} -> {pts[-1]["mean_auc"]:.4f}  ({slope}, {delta:+.4f})'
            )
