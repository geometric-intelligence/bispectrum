...
on July 24; every number below is measured, not projected. Raw results:
...
\textbf{Character counts} (markdown, before OpenReview formatting): global
3{,}745; yWuC 8{,}182; LgCU 3{,}101; AZ1B 5{,}819. The rebuttal limit is
10{,}000 characters per response, so all four fit as-is; the tightest is
yWuC with $\sim$1.8k headroom for edits.

\textbf{Review transcripts.} The quoted reviews below (including all inline
math, which the original copy-paste had dropped) were verified against
OpenReview screenshots on July 24 and match the posted reviews; they are safe
to quote in replies.

\textbf{Decisions needed from the group:}
(1) yWuC/global assert the hidden-PDF-text phrases are a NeurIPS watermark
inserted in reviewer-facing PDFs --- we should confirm this with the Program
Chairs \emph{before} posting, since we state it as fact;
...
(4) E1 has no consumer-GPU numbers yet (only A100 + host CPU); Johan is
running the Apple-silicon numbers locally
(\texttt{rebuttal/benchmark\_devices.py --device mps} and \texttt{--device
cpu}, writing \texttt{mac\_mps.json}/\texttt{mac\_cpu.json} per the runbook,
then rerun \texttt{analyze\_device\_benchmarks.py} with all JSONs to refresh
\texttt{combined.md}); a consumer-GPU volunteer is still needed before the
camera-ready.
\end{metacomment}
...
%=====================================================================
\section{Global response (post to all reviewers and AC)}
%=====================================================================
...
\item timings on an NVIDIA A100 (PyTorch 2.11, CUDA 13.0) and its x86-64 host
  ...
  ($\mathrm{SO}(3)$-on-$S^2$), so we will scope the ``sub-millisecond'' claim
  to GPU execution; and
...
reduces the \textbf{coefficient count} from $O(|G|^2)$ to $O(|G|)$; the
selective forward cost in Table~1 is $O(|G|^2)$), use 96.8\% consistently for
...
phrases and placements occur across NeurIPS 2026 submissions; the Program
...
measurements. We will also make the scope explicit: at $(8,16)$ channels the
methods are statistically equivalent ($74.5\pm0.6\%$ bispectrum versus
$74.3\pm1.5\%$ max), whereas the submitted high-capacity result is not
evidence for a universal advantage of complete pooling.
...
limitation'') is deliberately identical in the yWuC response and the global
...
and the low-data results. We agree that the original text did not separate
the theoretical status of the finite-group construction from that of the
spherical construction sharply enough.
...
\textbf{Band-limit and out-of-dataset reconstruction.} We agree that
validating only at $L=12$ while classifying at $L=15$ was a gap. We repeated
...
residual 0.27, IQR $[0.21, 0.52]$; successful cases sit at the SHT
...
(median aligned residual 1.19), so the joint success rate is 0\% and we
report it as such rather than adjusting thresholds post hoc; we cannot
...
NVIDIA A100 80GB (PyTorch 2.11.0, CUDA 13.0) and its x86-64 host CPU (same
PyTorch build):
...
We agree that ``likely overfits'' was not established by test accuracy alone.
...
bispectral with weight decay $10^{-3}$/$10^{-2}$, dropout 0.2/0.5, and
...
the bispectral head at this width, not memorization; we will replace the
speculative sentence in the paper with this evidence. This experiment is also
...
we do not claim superiority to those methods. The revised related-work
...
\item ``consistently outperform'' will be replaced by the narrower observed
  ...
  elsewhere;
\item coverage will be reported consistently as 96.8\%; and
...
(b) the portability table now says A100, not H100/consumer GPU as the draft
skeleton assumed --- the H100 numbers exist in the paper itself, and Johan is
running the Apple-silicon (MPS + Mac CPU) benchmark locally, so the combined
revision table will have H100 + A100 + x86 CPU + MPS; if the Mac numbers land
before the discussion deadline we can post them as a follow-up comment to
yWuC's Q4;
(c) at 8{,}182 characters this response fits the 10{,}000-character limit
with $\sim$1.8k headroom; if edits (e.g.\ adding the Mac MPS numbers) push it
over, the ``Baselines and scope'' and ``Corrections'' sections compress best
(the corrections are repeated in the global response).
\end{metacomment}
...
only in prose, and will move them there. We did not run S2CNN in the
submitted experiments, so we cannot infer an apples-to-apples speed advantage
from its published accuracies. A reliable matched re-run was not feasible
during the response window; we will retain the accuracy comparison,
explicitly label it as published, and remove any implication of a measured
...
re-run would be less credible than the explicit scope statement. If AZ1B
