...
\textbf{Character counts} (reflowed paste blocks in
\texttt{openreview\_paste.md}): global 4{,}281; yWuC 9{,}099; LgCU 3{,}218;
AZ1B 6{,}669. The Program Chairs re-opened the global Official Comment on
July 28, so a global comment is posted after all, alongside the
self-contained per-reviewer responses. The limit is 10{,}000 characters per
box, so all four fit as-is; the tightest is yWuC with $\sim$0.9k headroom
for edits.
...
The posted version is the GLOBAL block in
\texttt{rebuttal/openreview\_paste.md} (4{,}281 characters), which updates
this section's numbers, adds the camera-ready baseline commitments, and
includes the watermark clarification for the AC. The per-reviewer responses
remain self-contained; this section stays the consistency checklist. Content
distribution across the responses:
