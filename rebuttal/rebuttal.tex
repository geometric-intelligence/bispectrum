...
3{,}895; yWuC 8{,}339; LgCU 3{,}101; AZ1B 5{,}819. The rebuttal limit is
10{,}000 characters per response, so all four fit as-is; the tightest is
yWuC with $\sim$1.7k headroom for edits.
...
\item timings at batch 16, float32 on an NVIDIA A100 (PyTorch 2.11, CUDA
  ...
  the claim to those settings; and
...
all six configurations run on MPS and are sub-millisecond. The H100 numbers
remain in the paper itself, and a consumer-GPU volunteer is still needed; the
Mac results can be posted as a follow-up comment to yWuC's Q4;
(c) at 8{,}339 characters this response fits the 10{,}000-character limit
with $\sim$1.7k headroom; if further edits push it
