...
The submitted single-seed Table 11 values (68.5% and 78.5%) fall within these
cross-seed ranges. A train/validation gap never opens: train accuracy stays at or below
...
with the optimization/capacity conclusion supported by these measurements. The
revision will also state the scope plainly: at $(8,16)$ channels the methods
are statistically equivalent ($74.5\pm0.6\%$ bispectrum versus $74.3\pm1.5\%$
max), and the high-capacity result identifies a regime where max pooling is
...
- **Clebsch–Gordan coefficients** give the change of basis decomposing the
  ...
  what makes each bispectral scalar rotation invariant.
