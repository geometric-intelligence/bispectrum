...
\begin{enumerate}
  \item \textbf{Library.} PyTorch \texttt{nn.Module}s for seven
    $G$/domain pairs with a uniform API; selective $O(|G|)$ default,
    precomputed CG buffers, GPU-optimized forward passes
    ($<$1\,ms for finite groups), 96.8\% test coverage.
  \item \textbf{Theory.} A selective $\SO(3)$ invariant on $S^2$,
    $\Phi_{\mathrm{sel}}$, with $\Theta(L^2)$ coefficients
    (Sec.~\ref{sec:so3-selective}), matching the orbit-space lower
    bound $(L{+}1)^2 - 3$; reconstruction evidence supports that it
    preserves substantial orbit information, but a formal
    completeness proof is left to future work.
  \item \textbf{Benchmarks.} PCam under $C_8$, OrganMNIST3D under
    the octahedral group, and Spherical MNIST under $\SO(3)$,
    against norm / gated / Fourier-ELU / max pooling and augmented
    CNN baselines; near-exact invariance and a data-efficiency
    advantage over incomplete alternatives at low data.
\end{enumerate}
...
\begin{table}[t]
  ...
  \begin{tabular}{lccccc}
    ...
    \midrule
    \textbf{Selective bispectrum} (finite $G$) & \textbf{Yes} & \textbf{Yes}
...
\texttt{\{Group\}on\{Domain\}} (e.g.\ \texttt{SO3onS2}) with a
