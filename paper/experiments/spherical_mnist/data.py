...
def project_to_sphere(
    ...
    for i in range(0, images.shape[0], batch_size):
        ...
        proj = F.grid_sample(
            batch, g, mode='bilinear', padding_mode='zeros', align_corners=True,
        )
...
class SphericalMNISTDataset(Dataset):
    ...
    Precomputes and caches the spherical projection (and optional rotation)
    so that subsequent runs load instantly.
    """
...
def get_dataloaders(
    ...
    train_ds = SphericalMNISTDataset(
        'train', data_dir, nlat, nlon, rotated=train_rotated, seed=seed,
    )
    val_ds = SphericalMNISTDataset(
        'val', data_dir, nlat, nlon, rotated=train_rotated, seed=seed,
    )
    ...
    test_nr_loader = DataLoader(
        test_nr_ds, batch_size=batch_size, shuffle=False,
        num_workers=0, pin_memory=True,
    )
    test_r_loader = DataLoader(
        test_r_ds, batch_size=batch_size, shuffle=False,
        num_workers=0, pin_memory=True,
    )
