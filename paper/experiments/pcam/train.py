...
def evaluate_rotation_robustness(
    ...
    """Evaluate model consistency across input rotations.

    For each angle, rotate all test images and measure AUC. An equivariant model should produce
    consistent AUC regardless of angle.
    ...
    for angle in angles:
        ...
        results[angle] = _compute_auc(all_labels, all_probs)
...
def run_sweep(args: argparse.Namespace):
    ...
    for model_name in models:
        for seed in seeds:
            ...
            args.seed = seed
            print(f'\n{"=" * 60}')
            print(f'Running: model={model_name}, group={args.group}, seed={seed}')
            print(f'{"=" * 60}')
            results = train(args)
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help='Fraction of training data to use (for data-efficiency experiments).',
    )
    parser.add_argument(
        '--sweep',
        action='store_true',
