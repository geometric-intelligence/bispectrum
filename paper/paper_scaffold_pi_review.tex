...
map. Five model variants share the same backbone, differing only in the
nonlinearity and terminal invariant pooling.  A sixth, backbone-free
variant showcases \texttt{SO2onDisk} as a standalone feature extractor:
...
\begin{itemize}
  ...
  \item \textbf{Disk bispectrum}: no convolutional backbone;
    \texttt{SO2onDisk} is applied per RGB channel to the raw $96\times 96$
    patches, producing rotation-invariant Fourier--Bessel bispectrum
    coefficients that are fed into a small MLP classifier.
\end{itemize}
...
\begin{table}[t]
  \caption{PCam results at the ${\sim}100$K parameter budget (10\% training
    data, seed 42). The first five models share a DenseNet backbone;
    the disk bispectrum model has no backbone and uses \texttt{SO2onDisk}
    directly on the input patches.}
  ...
  \begin{tabular}{lrcc}
    ...
    \midrule
    Disk bisp.\ (\texttt{SO2onDisk}) & 100K & 0.794 & 0.724 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\emph{Disk bispectrum as a backbone-free baseline.}
The \texttt{SO2onDisk} model applies the selective disk bispectrum
(Definition~4.2 of Myers \& Miolane) directly to the raw input patches
without any learned convolutional features.  At 100K parameters
(bandlimit~$=10$) it reaches AUC~0.794---substantially below the
learned models, as expected for a handcrafted feature extractor.
However, its rotation robustness is near-perfect:
$\sigma_{\text{AUC}} < 2 \times 10^{-5}$ across 12 test-time rotation
angles, compared to residual angle-dependent variation in the $C_8$
models.  At 1\% training data, the disk bispectrum retains AUC~0.778,
showing that the analytically rotation-invariant features degrade
gracefully when data is scarce.  This variant demonstrates that
\texttt{SO2onDisk} can serve as a drop-in rotation-invariant feature
extractor from the library, independent of any equivariant backbone.
