...
|---|---|---|---|
| `CnonZn` | C_n | ℤ/nℤ | Cyclic rotations on a discrete cycle |
| `DnonR2` | D_n | ℝ² | Dihedral symmetries (rotations + reflections) on 2D plane |
| `SO3onS2` | SO(3) | S² | 3D rotations on the 2-sphere |
| `OonR3` | O | ℝ³ | Octahedral symmetries on 3D space *(future)* |
...
class {Group}on{Domain}(nn.Module):
    ...
    def index_map(self) -> list[tuple]:
        ...
        Allows interpreting which (ρ₁, ρ₂, ρ) each output corresponds to.
        """
```
...
### `CnonZn(n: int)` — Cyclic group C_n on ℤ/nℤ

**Mathematical setting:**
Signal f: ℤ/nℤ → ℝ. Group C_n acts by cyclic shift: (T_g f)(x) = f(x - g mod n).

**Bispectrum formula** (commutative case, [Kakarala 2009]):
```
β(f)_{k₁,k₂} = f̂_{k₁} · f̂_{k₂} · f̂*_{k₁+k₂ mod n}
```
where f̂_k = Σ_{x=0}^{n-1} e^{-i2πkx/n} f(x) is the DFT.

**Selective coefficients** (Algorithm 1, [Mataigne et al. 2024]):
Full bispectrum has n² coefficients. Selective version needs only n:
```
{ β_{0,0}, β_{0,1}, β_{1,1}, β_{1,2}, ..., β_{1,n-2} }
```
...
### `DnonR2(n: int)` — Dihedral group D_n on ℝ²

**Mathematical setting:**
Signal f: G → ℝ where G = D_n = ⟨a, x | aⁿ = x² = e, xax = a⁻¹⟩.
D_n acts on ℝ² via rotations (a: rotation by 2π/n) and reflections (x: flip).

**Irreducible representations:**
- 2D irreps ρ_k, k = 1, ..., ⌊(n-1)/2⌋:
  ρ_k(aˡ xᵐ) = Rot(2πlk/n) · Refl^m
- 1D irreps: ρ₀ (trivial), ρ₀₁, and (for n even) ρ₀₂, ρ₀₃

**Selective coefficients** (Algorithm 3, [Mataigne et al. 2024]):
Only ⌊(n-1)/2⌋ + 2 matrix-valued bispectral coefficients needed ≈ 4|D_n| scalars.

**Usage:**
```python
bsp = DnonR2(n=4)
f = torch.randn(batch_size, 8)           # signal on D_4 (|D_4| = 8), shape (batch, 2n)
output = bsp(f)                           # shape (batch, output_size), complex64
```
...
### `SO3onS2(lmax: int)` — SO(3) on S²

**Mathematical setting:**
Signal f: S² → ℝ. Group SO(3) acts by 3D rotation.

**Bispectrum formula** [Kakarala 1992, Cohen et al.]:
```
β(f)_{l₁,l₂}[l] = (F_{l₁} ⊗ F_{l₂}) C_{l₁,l₂} F̂_l†
```
where F_l are spherical harmonic coefficients and C_{l₁,l₂} are Clebsch-Gordan matrices.
...
**Q3: SO3onS2 grid parameters**
Currently SO3onS2 uses external `RealSHT` which requires `(nlat, nlon)`. Should these be constructor params or inferred from input at forward time?
*Lean toward*: constructor params for efficiency (SHT is precomputed).

**Q4: Inversion as a separate method or separate class?**
The inversion algorithms (recovering f from β) are mathematically significant. Options:
